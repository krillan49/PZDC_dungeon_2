...
def temporary_patch_concentration
  if @name_passive_pl == "Концентрация"
    damage_passive_pl = rand(0..0.1 * (@max_mp_pl * (1 + 0.05 * @lvl_passive_pl) - 100))
    @lor_passive_pl = "(#{@lvl_passive_pl}): если мана больше 100(#{@max_mp_pl}) наносится случайный доп урон до #{(0.1 * (@max_mp_pl * (1 + 0.05 * @lvl_passive_pl) - 100)).round(1)}"
  end
end
...
def character_panel
  ...
  puts '--------------------------------------------------------------------------------------------'
  puts "#{@name_pl}"
  puts "Уровень #{@lvl_pl} (#{@exp_pl}/#{@exp_lvl[@lvl_pl + 1]})"
  puts "Н А В Ы К И:"
  ...
  puts "С Т А Т Ы:"
  puts "HP #{@hp_pl.round}/#{@max_hp_pl} Реген #{@regen_hp_pl_base} Восстановление #{@recovery_hp_pl.round}"
  puts "MP #{@mp_pl.round}/#{@max_mp_pl} Реген #{@regen_mp_pl_base} Восстановление #{@recovery_mp_pl.round}"
  puts "Урон #{@mindam_pl}-#{@maxdam_pl} (базовый #{@mindam_pl_base}-#{@maxdam_pl_base} + #{@weapon} #{@mindam_weapon}-#{@maxdam_weapon})"
  puts "Точность #{@accuracy_pl} (базовая #{@accuracy_pl_base} + #{@gloves} #{@accuracy_gloves})"
  puts "Броня #{@armor_pl} (базовая #{@armor_pl_base} + #{@torso} #{@armor_torso} + #{@helmet} #{@armor_helmet} + #{@gloves} #{@armor_gloves} + #{@shield} #{@armor_shield})"
...
when 'F'
  ...
  effect_noncombat_pl = (@max_hp_pl - @hp_pl) * 0.2 * coeff_noncombat_pl
  noncombat_mp_cost_pl = 10
  @lor_noncombat_pl = "(#{lvl_noncombat_pl}): восстанавливает #{((@max_hp_pl - @hp_pl) * 0.2 * coeff_noncombat_pl).round} жизней, чем больше жизней потеряно, тем больше эффект(#{(0.2 * coeff_noncombat_pl * 100).round}%), цена 10 маны."
when 'T'
...
@armor_pl = @armor_pl_base + @armor_torso + @armor_helmet + @armor_gloves + @armor_shield
...
while true
  ...
  while points != 0
    ...
    while distribution != 'H' and distribution != 'M' and distribution != 'X' and distribution != 'A'
      puts "Распределите очки характеристик. У вас осталось #{points} очков"
  ...
  while skil_points != 0
    ...
    while distribution != 'S' and distribution != 'P' and distribution != 'N'
      puts "Распределите очки навыков. У вас осталось #{skil_points} очков"
  ...
  # Навык Первая помощь ---------------------------------------------------------------------------------
  if (@max_hp_pl - @hp_pl) > 0 and @name_noncombat_pl == "Первая помощь"
    coeff_noncombat_pl = 1 + 0.1 * lvl_noncombat_pl
    effect_noncombat_pl = (@max_hp_pl - @hp_pl) * 0.2 * coeff_noncombat_pl
    print "У вас #{@hp_pl.round}/#{@max_hp_pl} жизней, хотите использовать навык #{@name_noncombat_pl}, чтобы восстановить #{effect_noncombat_pl.round} жизней за 10 маны? (Y/N) "
    ...
    if noncombat_choice == "Y" and @mp_pl >= noncombat_mp_cost_pl
      ...
      puts "Вы восстановили #{effect_noncombat_pl.round} жизней за #{noncombat_mp_cost_pl} маны, теперь у вас #{@hp_pl.round}/#{@max_hp_pl} жизней и #{@mp_pl.round}/#{@max_mp_pl} маны"
  ...
  if run == false
    ...
    if stash_magic_treasure <= 10 and stash_magic >= 180
      ...
      bonus_skil_points = 1
      puts "Книга умений. Ваши очки умений увеличились на #{bonus_skil_points}"
      skil_points += bonus_skil_points
  ...
  for i in 0...@exp_lvl.size
    if @exp_pl >= @exp_lvl[i] and @lvl_pl < i
      new_points = i - @lvl_pl
      points += new_points
      new_skil_points = i - @lvl_pl
      skil_points += new_skil_points
      @lvl_pl += (i - @lvl_pl)
      puts "Вы получили новый уровень, теперь ваш уровень #{@lvl_pl}й"
      puts "Вы получили #{new_points} очков характеристик и #{new_skil_points} очков навыков"
      puts "У вас теперь #{points} очков характеристик и #{skil_points} очков навыков"
    end
  end
